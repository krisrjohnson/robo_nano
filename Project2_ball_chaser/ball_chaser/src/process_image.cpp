...
// http://docs.ros.org/melodic/api/sensor_msgs/html/msg/Image.html
    // data is a matrix of size(step*rows). Step/3 is pixels in a row.
void process_image_callback(const sensor_msgs::Image img)
{

    int pixel = -1, white_pixel = 255;
    float pixel_x_coord = -1;
    ...
    if (pixel >= 0) {
        pixel_x_coord = float(pixel % img.step) / img.step;  //one float should cast the rest
	...
	if (pixel) {
		...
		ROS_INFO_STREAM("pixel: " << sPixel << "\npixels coordinate: " << sPixelCoord); 
	} else {
         	ROS_INFO_STREAM("no pixel found"<<std::endl);
        }


	//angular_z>0 is to the left (counter-clockwise); angular_z<0 to the right (clockwise)
        if (pixel_x_coord == -1) {
            drive_robot(0.0, 0.0);
        } else {
